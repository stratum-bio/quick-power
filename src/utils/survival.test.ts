...
describe('baselineToTreatmentSurvival', () => {
  it('should calculate treatment survival correctly', () => {
    ...
    expect(baselineToTreatmentSurvival(baseSurv, hazardRatio)).toBeCloseTo(expected);
  });

  it('should return baseSurv if hazardRatio is 1', () => {
    const baseSurv = 0.7;
    const hazardRatio = 1;
    expect(baselineToTreatmentSurvival(baseSurv, hazardRatio)).toBeCloseTo(baseSurv);
  });
...
describe('evaluateExponential', () => {
  it('should evaluate exponential survival probabilities correctly', () => {
    const time = [1, 2, 3];
    const lambda = 0.1;
    const expected = [Math.exp(-0.1 * 1), Math.exp(-0.1 * 2), Math.exp(-0.1 * 3)];
    expect(evaluateExponential(time, lambda)).toEqual(expect.arrayContaining(expected.map(val => expect.closeTo(val))));
  });
...
describe('evalExponentialCurve', () => {
  it('should evaluate exponential curve with correct number of points and values', () => {
    ...
    expect(result[result.length - 1].time).toBeCloseTo(Math.max(...originalTime));
...
describe('estimateConfidence', () => {
  it('should return the expected structure with default values', () => {
    ...
    const simCount = 1000;
    const result = estimateConfidence(baseMedianTTE, treatMedianTTE, sampleSize, simCount);
