...
import { FactorType, type FactorQuery, type ParsedFactor } from "../types/demo_types.d";
...
export function toRegularGrid(dist: CumulativeDist, minVal: number, maxVal: number): CumulativeDist {
  const values = Array.from({ length: maxVal - minVal + 1 }, (_, idx) => minVal + idx);
  const probabilities: number[] = Array.from({length: values.length}, () => 0.0);
...
export function computeCumulativeDists(factors: ParsedFactor[], expectedMax: number): CumulativeDist[] {
  if (factors.length === 0) {
    return [];
  }
...
function queryToCumulativeDist(query: FactorQuery, expectedMax: number): CumulativeDist {
  // this is sorted already
  const values = query.intervals.map((interval) => interval.upper ?? expectedMax);
...
function sortIntervals(factors: ParsedFactor[]): ParsedFactor[] {
  return factors.sort((a, b) => (a.value_range.lower ?? -1) - (b.value_range.lower ?? -1));
}

function scoreAge(factors: ParsedFactor[], query: FactorQuery): number {
  const sortedFactors = sortIntervals(factors);
  const cumulativeDists = computeCumulativeDists(sortedFactors, 90).map((d) => toRegularGrid(d, 30, 90));
...
function scoreECOG(factors: ParsedFactor[], query: FactorQuery): number {
  const sortedFactors = sortIntervals(factors);
  const cumulativeDists = computeCumulativeDists(sortedFactors, 3).map((d) => toRegularGrid(d, 0, 3));
...
export function searchTrials(trials: TrialMeta[], disease: string, factor: FactorType, query: FactorQuery): TrialMeta[] {
  const diseaseTrials = trials.filter((t) => t.disease === disease);
  const withFactorTrials = diseaseTrials.filter((t) => t.data_index !== null && hasFactor(t.data_index, factor))
