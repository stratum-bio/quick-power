...
describe('decomposition', () => {
  ...
  describe('fit_reference_survival', () => {
    it('should return a KaplanMeier object', () => {
      ...
      const result = fit_reference_survival(mockKaplanMeier, proportions, hazard_ratios);
      expect(result).toHaveProperty('time');
      expect(result).toHaveProperty('probability');
      expect(result.time).toEqual(mockKaplanMeier.time);
      expect(result.probability.length).toEqual(mockKaplanMeier.probability.length);
    });
    ...
    it('should converge for a simple case', () => {
      ...
      expect(result.probability[result.probability.length - 1]).toBeGreaterThan(0);
    });
    ...
    it('should throw an error if proportions length is not equal to hazard_ratios length', () => {
      ...
      const hazard_ratios = [1, 0.5];
      expect(() => fit_reference_survival(mockKaplanMeier, proportions, hazard_ratios)).toThrow(
        "Proportions and hazard ratios must match"
      );
    });
  ...
  describe('recompose_survival', () => {
    ...
    it('should recompose survival correctly for a simple case', () => {
      ...
      for (let i = 0; i < result.probability.length - 1; i++) {
        expect(result.probability[i]).toBeGreaterThanOrEqual(result.probability[i + 1]);
        expect(result.probability[i]).toBeLessThanOrEqual(1.0);
        expect(result.probability[i]).toBeGreaterThanOrEqual(0.0);
      }
      expect(result.probability[result.probability.length - 1]).toBeGreaterThanOrEqual(0);
    });
