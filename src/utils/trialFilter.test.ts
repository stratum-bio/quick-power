...
describe("toRegularGrid", () => {
  it("should convert a simple cumulative distribution to a regular grid", () => {
    ...
    const expectedRegularGridValues = Array.from(
      { length: 31 },
      (_, idx) => 0 + idx,
    );
    ...
    for (let i = 0; i <= 30; i++) {
      if (i <= dist.values[0]) {
        // Before or at the first point
        expectedRegularGridProbs.push(
          interpolate(i, 0, 0, dist.values[0], dist.probabilities[0]),
        );
      } else if (i <= dist.values[1]) {
        ...
        expectedRegularGridProbs.push(
          interpolate(
            ...
            dist.values[1],
            dist.probabilities[1],
          ),
        );
      } else if (i <= dist.values[2]) {
        ...
        expectedRegularGridProbs.push(
          interpolate(
            ...
            dist.probabilities[1],
            dist.values[2],
            dist.probabilities[2],
          ),
        );
      } else {
        // After the last point
        expectedRegularGridProbs.push(
          dist.probabilities[dist.probabilities.length - 1],
        );
      }
    }
