...
const TrialDetail: React.FC = () => {
  ...
  return (
    <div className="p-6 text-black text-left w-full">
      ...
      <div className="mb-4">
        <p><span className="font-semibold">PubMed ID:</span> {trialData.meta.pubmed}</p>
        <p><span className="font-semibold">Publication Date:</span> {trialData.meta.publication_date.split(' ')[0]}</p>
        <p><span className="font-semibold">Disease:</span> {trialData.meta.disease}</p>
        <p><span className="font-semibold">Subjects:</span> {trialData.meta.subjects}</p>
        <p><span className="font-semibold">Arms:</span> {trialData.meta.arms}</p>
      </div>
        ...
        <div className="space-y-4 w-96">
          {trialData.arms.map((arm, index) => (
            <div key={index} className="border p-4 rounded-md">
              <h3 className="text-xl font-semibold mb-2">Arm Name: {arm.arm_name}</h3>
              <p><span className="font-semibold">Number of Events:</span> {arm.events.filter(Boolean).length}</p>
              <p><span className="font-semibold">Number of Time Points:</span> {arm.time.length}</p>
            </div>
          ))}
        </div>
      ) : (
